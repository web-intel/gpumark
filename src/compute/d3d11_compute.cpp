...
int __cdecl main()
{
...
#ifdef USE_SLM_8X8_4X16
    if ( FAILED( CreateComputeShader( L"../../src/compute/slm_8x8_4x16.hlsl", "CSMain", g_pDevice, &g_pCS ) ) )
        return 1;
#else
    if ( FAILED( CreateComputeShader( L"../../src/compute/slm_4x4_16x16.hlsl", "main", g_pDevice, &g_pCS ) ) )
        return 1;
#endif // USE_SLM_8X8_4X16
    ...
    for (int it = 0; it < computeCount; it++)
    {
        ...
        float kernel_time = (Delta / Frequency) * 1000000.0f;
