...
void OrbitCamera::GetAllData(float dataBuf[11]) const {
    XMFLOAT4 v2F;
    XMStoreFloat4(&v2F, mCenter);
    dataBuf[0] = v2F.x;
    dataBuf[1] = v2F.y;
    dataBuf[2] = v2F.z;
    dataBuf[3] = v2F.w;

    dataBuf[4] = mMinRadius;
    dataBuf[5] = mMaxRadius;

    dataBuf[6] = mLatAngle;
    dataBuf[7] = mLongAngle;
    dataBuf[8] = mRadius;
...
void OrbitCamera::LoadData(float dataBuf[11]) {
    XMFLOAT4 v2F;
    v2F.x = dataBuf[0];
    v2F.y = dataBuf[1];
    v2F.z = dataBuf[2];
    v2F.w = dataBuf[3];
    mCenter = XMLoadFloat4(&v2F);

    mMinRadius = dataBuf[4];
    mMaxRadius = dataBuf[5];
    mLatAngle = dataBuf[6];
    mLongAngle = dataBuf[7];
    mRadius = dataBuf[8];
    mFov = dataBuf[9];
    mAspect = dataBuf[10];
}
