...
static void ImGui_ImplDX12_CreateFontsTexture(bool enableAlphaBlending)
{
    ...
    // Upload texture to graphics system
    {
        ...
        for (int y = 0; y < height; y++)
            memcpy((void *)((uintptr_t)mapped + y * uploadPitch), pixels + y * width * 4,
                   width * 4);
...
bool ImGui_ImplDX12_CreateDeviceObjects(int MSAACount, bool enableAlphaBlending)
{
    if (!g_pd3dDevice)
        return false;
    if (g_pPipelineState)
    ...
    // DX12 sample code but remove this dependency you can:
    ...
    // Create the vertex shader
    {
        ...
        D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_5_0", 0, 0,
        ...
        if (g_pVertexShaderBlob ==
            ...
            return false;
        psoDesc.VS = {g_pVertexShaderBlob->GetBufferPointer(),
                      g_pVertexShaderBlob->GetBufferSize()};

        // Create the input layout
        static D3D12_INPUT_ELEMENT_DESC local_layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0, IM_OFFSETOF(ImDrawVert, pos),
             D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, IM_OFFSETOF(ImDrawVert, uv),
             D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, IM_OFFSETOF(ImDrawVert, col),
             D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        };
    ...
    // Create the pixel shader
    {
        ...
        D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_5_0", 0, 0,
    ...
    // Create the rasterizer state
    {
        ...
        desc.MultisampleEnable      = MSAACount > 1;
    ...
    if (g_pd3dDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&g_pPipelineState)) !=
        S_OK)
        return false;
...
void ImGui_ImplDX12_NewFrame(int MSAACount, bool enableAlphaBlending)
{
    if (!g_pPipelineState)
        ImGui_ImplDX12_CreateDeviceObjects(MSAACount, enableAlphaBlending);
}
