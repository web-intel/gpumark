    ...
    int windowHeight)
{
    ...
    rtvHeapDesc.NumDescriptors             = mMSAACount > 1 ? mFrameCount + 1 : mFrameCount;
    ...
    if (mMSAACount > 1)
    {
        ...
        textureDesc.SampleDesc.Count    = mMSAACount;
                             ...
                             std::bitset<static_cast<size_t>(TOGGLE::TOGGLEMAX)> *toggleBitset)
{
    ...
    ImGui_ImplDX12_NewFrame(mMSAACount,
...
void ContextD3D12::createDepthStencilView()
{
    ...
    depthStencilViewDesc.ViewDimension =
        mMSAACount > 1 ? D3D12_DSV_DIMENSION_TEXTURE2DMS : D3D12_DSV_DIMENSION_TEXTURE2D;
    ...
    bool enableBlend) const
{
    ...
    psoDesc.RasterizerState.MultisampleEnable     = mMSAACount > 1;
