...
void D3D12nBodyGravity::Simulate()
{
	...
	if (GetParticleBufferSrvIndex () == 0)
	{
...
void D3D12nBodyGravity::MoveToNextFrame ()
{
	...
	if (m_queryReadbackIndex >= 0) {
		...
		auto titleString = std::to_string (averageDiffMs) + " ms (" + std::to_string ((1000.0 / averageDiffMs)) + " fps)" + " " + std::to_string (m_frameTimeEntryCount);
		...
		if (m_frameTimeEntryCount > 70) {
			std::wstringstream ws;
			ws << L"FPS:" << int(1000.0 / averageDiffMs * 100) / 100.0 << "\n";
			std::wstring str = ws.str();
			LPCWSTR lpBuff = const_cast<LPWSTR>(str.c_str());
			DWORD dwSize = 0;
